// This file contains the class definition for the odd even jump problem.
...
//=============================================================================
OddEvenJump::OddEvenJump(std::string command_id) { command_id_ = command_id; }

//=============================================================================
void OddEvenJump::Deserialize(tools::CSVParser& /*params*/) {
  // Nothing to see here.
}
...
class Solution {
 public:
  int FindClosestGreaterOrEqual(std::vector<int>& arr, int index) {
    int index_next = -1;

    for (int ii = index + 1; ii < (int)arr.size(); ii++) {
      if (index_next == -1) {
        if (arr[index] <= arr[ii]) {
          index_next = ii;
        }
      } else if (arr[index] <= arr[ii] && arr[ii] < arr[index_next]) {
        index_next = ii;
      }
    }
  ...
  int FindClosestLessOrEqual(std::vector<int>& arr, int index) {
    int index_next = -1;
  ...
  bool GoodStartingIndex(std::vector<int>& arr, int index) {
    int current_index = index;
    bool is_odd_jump = true;

    while (current_index != (int)arr.size() - 1 && current_index != -1) {
      if (is_odd_jump) {
        is_odd_jump = false;
        current_index = FindClosestGreaterOrEqual(arr, current_index);
      } else {
        is_odd_jump = true;
        current_index = FindClosestLessOrEqual(arr, current_index);
      }
    }
  ...
  int oddEvenJumps(std::vector<int>& arr) {
    int good_index_count = 0;

    // This  is slow, O(n!)
    for (int index = 0; index < (int)arr.size(); index++) {
      if (GoodStartingIndex(arr, index)) {
        good_index_count++;
      }
    }
...
//=============================================================================
int OddEvenJump::Execute() {
  LOG(INFO) << "###################################################";
  LOG(INFO) << "####               Odd Even Jump               ####";
  LOG(INFO) << "###################################################";

  {
    std::vector<int> arr{10, 13, 12, 14, 15};
    Solution mine;
    LOG(INFO) << "Should be 2: " << mine.oddEvenJumps(arr);
  }

  {
    std::vector<int> arr{2, 3, 1, 1, 4};
    Solution mine;
    LOG(INFO) << "Should be 3: " << mine.oddEvenJumps(arr);
  }
